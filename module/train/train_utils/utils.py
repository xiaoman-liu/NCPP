...
def genereate_feature_list(dict, mapping):
    feature_names_list = []
    for key1, value in dict.items():
        for feature_name, number in value:
            mapped_key = mapping.get(feature_name, feature_name)
            if number > 1:
                for i in range(1, number + 1):
                    feature_names_list.append(f"{mapped_key}_{i}")
            else:
...
def generate_output_path(configs):
    ...
    if configs["train_label"] and configs["train_label"] != "test":
        output_path = os.path.join(configs["parent_output_path"],"milestones", configs["train_label"],"/".join(configs["workload_names"]+ [configs["select_model"]] + [timestamp]))
    else:
...
def read_config(root_dir, output_path, config_path, module_path):
    # Generate absolute paths
    config_path = generate_abs_path(root_dir, config_path)
    output_path = generate_abs_path(root_dir, output_path)
    module_path = generate_abs_path(root_dir, module_path)
    rank_info_path = generate_abs_path(root_dir,"../../data/processed/")
    ...
    model_dict = configs["model_dict"]
    select_model = configs["select_model"]
    if configs["platform"] == "SPR":
        model_history_root_path = configs["spr_model_history_root_path"]
    elif configs["platform"] == "EMR":
        model_history_root_path = configs["emr_model_history_root_path"]
    else:
        logger.error(f"Error: platform {configs['platform']} is not supported.")
        sys.exit(1)
    model_history_dir = configs["model_history_path_mapping"][configs["workload_names"][0]]
    model_history_path = os.path.join(model_history_root_path, model_history_dir, configs["train_label"]).replace("\\", "/")
    model_history_path = generate_abs_path(root_dir, model_history_path)

    model_file = configs["model_config_for_workload_dict"][select_model].get(configs["workload_names"][0], "default.yaml")
    model_path = os.path.join(module_path, "/".join(model_dict[select_model].split(".")[:-2]), model_file).replace("\\", "/")
    model_config = read_yaml_file(model_path)
    ...
    pre_trained_model_path = generate_abs_path(root_dir, configs["pre_trained_model_path"])

    # Update configuration dictionary
    parent_output_path = output_path
    configs.update(model_config)
    configs.update(rank_info)
