...
class GroupFeatureEmbedding():
    ...
    def scale_normalize(self, feature):
        ...
        if not selected_feature.empty:
            # self.processed_features = pd.concat([self.processed_features, selected_feature], axis = 1)
        #result = (self.labels - self.labels.mean()) / self.labels.std()
        #self.labels = result.loc[:, np.isfinite(result).all()]
        # self.logger.info("Finish data normalization using Normalization Scaler")
