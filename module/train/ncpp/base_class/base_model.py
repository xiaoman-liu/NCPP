...
class BaseModel():
    ...
    def copy_model(self):
        if self.copy_train_model and self.train_with_all_data:
            # new_model_path = self.output_path + "/ncpp"
            new_model_path = self.output_path + "/" + self.train_label
            obj_path = self.model_history_path
            self.logger.info("start change ncpp name to: {}".format(new_model_path))
            shutil.move(self.output_path + "/ncpp", new_model_path)
            try:
                ...
                self.logger.info("start copy ncpp to: {}".format(obj_path))
                shutil.copytree(new_model_path, obj_path)
                self.logger.info("finish copy ncpp to: {}".format(obj_path))
                ...
                if choice == 'y':
                    ...
                    shutil.copytree(new_model_path, obj_path)
                    self.logger.info("finish copy ncpp from {}  \nto {}".format(new_model_path, obj_path))
                else:
                    self.logger.info("Not copy ncpp from {} \n to: {}".format(new_model_path, obj_path))

        else:
            self.logger.info("Not copy ncpp from {} \n to {}".format(self.output_path + "/ncpp", self.model_history_path))
