...
class GroupMultiAttenResNet(BaseModel):
    ...
    def build_model(self):
        ...
        # model = Model(inputs={"mem_char_input": mem_char_var_input, "mem_numer_input": mem_numeric_var_input,
    ...
    def train(self):
        ...
        if self.use_pre_trained_model:
            ...
            if (self.verbose == True):
                pass
                # self.model.summary()
        # self.model.save_weights(self.output_path + '/fcn/model_init.hdf5')
        # self.logger.info(
        ...
        if self.is_save_model:
            if self.train_with_all_data:
                ...
                save_path = os.path.join(self.kth_fold_save_path, "model").replace("\\", "/")
    ...
    def predict(self):
        model_save_path = os.path.join(self.final_path, "model").replace("\\", "/")
        ...
        # y_pred = self.model.predict((self.char_x_test, self.numer_x_test))
