...
class oldMultiHeadAtten(Layer):
    ...
    def __init__(self, output_dim, nheads, return_attention = False,**kwargs):
        self.supports_masking = True
        self.return_attention = return_attention
        self.output_dim = output_dim
    ...
    def build(self, input_shape):
        ...
        # inputs.shape = (batch_size, time_steps, seq_len)
        self.kernel = self.add_weight(name='kernel',
                                      shape=(3, input_shape[2], self._n_heads * self.output_dim),
                                      initializer='glorot_uniform',
                                      trainable=True)
        self.WO = self.add_weight(name='output',
                                      shape=(self.output_dim, self._n_heads),
                                      initializer='glorot_uniform',
                                      trainable=True)
    ...
    def split_heads(self, x):
        ...
        """
        x = K.reshape(x, (-1, 1, self._n_heads, self.output_dim))
    ...
    def call(self, x, mask = None):
        ...
        # [batch_size, 1, Q_sequence_length * HEADS]


        # Qi = K.squeeze(Qi, axis=1)
        # Ki = K.squeeze(Ki, axis=1)
        # Vi = K.squeeze(Vi, axis=1)
        #(None, 1, 128, 2)
        print("qi shape", Qi.shape)

        # self.kernel("WQ.shape", WQ.shape)
        Qi = self.split_heads(Qi)
        Ki = self.split_heads(Ki)
        Vi = self.split_heads(Vi)
        # Vi = K.squeeze(Vi, axis=1)
        ...
        print("K.permute_dimensions(Qi, [0, 2, 3, 1])", K.permute_dimensions(Qi, [0, 2, 3, 1])) # [batch_size,  HEADS,  q_seq, 1]
        print("K.permute_dimensions(Ki, [0, 2, 1, 3])", K.permute_dimensions(Ki, [0, 2, 1, 3])) #  # [batch_size,  HEADS,  1, k_seq, ]
        # QK = K.batch_dot(K.permute_dimensions(Qi, [0, 2, 1, 3]), K.permute_dimensions(Ki, [0, 2, 1, 3]), axes=[3, 3])
        # print("QK.shape", QK.shape)

        QK = tf.matmul(K.permute_dimensions(Qi, [0, 2, 3, 1]), K.permute_dimensions(Ki, [0, 2, 1, 3])) # [batch_size,  HEADS,  q_seq, k_seq]
        # [batch_size,self.multiheads,Q_sequence_length,K_sequence_length]
        # QK = K.permute_dimensions(QK, pattern=[0, 3, 2, 1]) #[batch_size,  k_seq, q_seq, HEADS]

        print("tf QK.shape", QK.shape)
        # print("QK", K.eval(QK))
        ...
        # vi (None, 1, 3, 128)
        Vi = K.permute_dimensions(Vi, [0, 2, 3, 1])
        print("PER Vi.shape", Vi.shape)
        out = tf.matmul(QK, Vi)
        print("out.shape", out.shape)
        # out = K.squeeze(out, axis=3)
        out = K.batch_dot(out, WO, axes=[1, 2])
    ...
    def compute_output_shape(self, input_shape):
        ...
        return (input_shape[0], input_shape[1], self.output_dim)
    def get_config(self):
        config = super(oldMultiHeadAtten, self).get_config()
        config.update({
            'output_dim': self.output_dim,
            'return_attention': self.return_attention,
